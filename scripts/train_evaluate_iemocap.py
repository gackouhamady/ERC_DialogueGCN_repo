...
from models import MaskedNLLLoss # type: ignore
from models import DialogRNNModel, LSTMModel, GRUModel, DialogueGCNModel # type: ignore
from models import DialogueRNN # type: ignore
from models import pad, edge_perms, batch_graphify # type: ignore
from models import attentive_node_features, classify_node_features # type: ignore
from models import GraphNetwork # type: ignore
from sklearn.metrics import f1_score, accuracy_score, classification_report, precision_recall_fscore_support # type: ignore
...
class IEMOCAPDataset(Dataset): # type: ignore
    ...
    def collate_fn(self, data):
        dat = pd.DataFrame(data) # type: ignore
        return [pad_sequence(dat[i]) if i<4 else pad_sequence(dat[i], True) if i<6 else dat[i].tolist() for i in dat] # type: ignore
